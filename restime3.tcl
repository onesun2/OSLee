...
for {set k $min} {$k < $max} {fincr k $binsize} {
	set sum [expr $sum+$bin($k)]
}

for {set k $min} {$k < $max} {fincr k $binsize} {
#	puts "bin([expr ($k+($k+$binsize))/2]) = $bin($k)"
	puts "[expr (($k+($k+$binsize))/2)-1] : $bin($k)"
	set tn [expr ((($k+($k+$binsize))/2)-1)*($bin($k)/$sum)]
	set rtime [expr $rtime + $tn]
#	puts "tn = $tn, rtime = $rtime"
}
puts "residence time = $rtime"
set sum 0.0
